...
const DECK_PATH = resolve(STATIC_DIRECTORY, 'decks/m1-agentic-iac-fundamentals.html');
const ARTIFACT_DIRECTORY = resolve(REPOSITORY_ROOT, '.artifacts/decks/m1');
...
async function writeContactSheet() {
  const tiles = slides.map((slide) => {
    ...
    return `<figure><img src="slide-${number}.png" alt="Slide ${slide.n}: ${escapeHtml(slide.title)}"><figcaption><b>M1.${number}</b> ${escapeHtml(slide.title)}</figcaption></figure>`;
  }).join('\n');
  const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Module 1 deck contact sheet</title><style>body{margin:24px;background:#eee;font:15px system-ui;color:#222}h1{margin:0 0 20px}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(360px,1fr));gap:18px}figure{margin:0;background:white;padding:8px;border:1px solid #bbb;border-radius:8px}img{display:block;width:100%;aspect-ratio:16/9;object-fit:contain;background:white}figcaption{padding:8px 4px 3px;line-height:1.35}</style></head><body><h1>Module 1 · 61-slide visual QA</h1><div class="grid">${tiles}</div></body></html>`;
  await writeFile(join(ARTIFACT_DIRECTORY, 'contact-sheet.html'), html, 'utf8');
}

export async function renderM1Deck() {
  ...
  try {
    for (let index = 0; index < slides.length; index += 1) {
      ...
      const url = `http://127.0.0.1:${address.port}/decks/m1-agentic-iac-fundamentals.html?transition=none#/${index}${finalFragment}`;
  ...
  console.log(`wrote ${ARTIFACT_DIRECTORY}/slide-01.png through slide-61.png`);
  console.log(`wrote ${ARTIFACT_DIRECTORY}/contact-sheet.html`);
}

const isCli = process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url);
if (isCli) await renderM1Deck();
