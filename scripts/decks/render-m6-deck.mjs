...
const DECK_PATH = resolve(STATIC_DIRECTORY, 'decks/m6-build-test-optimize-reliable-iac-agent-workflows.html');
const ARTIFACT_DIRECTORY = resolve(REPOSITORY_ROOT, '.artifacts/decks/m6');
...
export async function renderM6Deck() {
  ...
  try {
    for (let index = 0; index < slides.length; index += 1) {
      ...
      const url = `http://127.0.0.1:${address.port}/decks/m6-build-test-optimize-reliable-iac-agent-workflows.html?transition=none#/${index}${finalFragment}`;
