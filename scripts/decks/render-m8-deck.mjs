...
const DECK_PATH = resolve(STATIC_DIRECTORY, 'decks/m8-test-secure-ai-generated-infrastructure.html');
const ARTIFACT_DIRECTORY = resolve(REPOSITORY_ROOT, '.artifacts/decks/m8');
...
export async function renderM8Deck() {
  ...
  try {
    for (let index = 0; index < slides.length; index += 1) {
      ...
      const url = `http://127.0.0.1:${address.port}/decks/m8-test-secure-ai-generated-infrastructure.html?transition=none#/${index}${finalFragment}`;
  ...
  console.log(`wrote ${ARTIFACT_DIRECTORY}/slide-01.png through slide-75.png`);
  console.log(`wrote ${ARTIFACT_DIRECTORY}/contact-sheet.html`);
}

const isCli = process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url);
if (isCli) await renderM8Deck();
