...
const DECK_PATH = resolve(STATIC_DIRECTORY, 'decks/m9-deploy-applications-kubernetes-helm-ai-agents.html');
const ARTIFACT_DIRECTORY = resolve(REPOSITORY_ROOT, '.artifacts/decks/m9');
...
export async function renderM9Deck() {
  ...
  try {
    for (let index = 0; index < slides.length; index += 1) {
      ...
      const url = `http://127.0.0.1:${address.port}/decks/m9-deploy-applications-kubernetes-helm-ai-agents.html?transition=none#/${index}${finalFragment}`;
