...
const DECK_PATH = resolve(STATIC_DIRECTORY, 'decks/m10-deliver-infrastructure-gitops-human-approval.html');
const ARTIFACT_DIRECTORY = resolve(REPOSITORY_ROOT, '.artifacts/decks/m10');
...
async function writeContactSheet() {
  const tiles = slides.map((slide) => {
    ...
    return `<figure><img src="slide-${number}.png" alt="Slide ${slide.n}: ${escapeHtml(slide.title)}"><figcaption><b>M10.${number}</b> ${escapeHtml(slide.title)}</figcaption></figure>`;
  }).join('\n');
  const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"><title>Module 10 deck contact sheet</title><style>body{margin:24px;background:#eee;font:15px system-ui;color:#222}h1{margin:0 0 20px}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(360px,1fr));gap:18px}figure{margin:0;background:white;padding:8px;border:1px solid #bbb;border-radius:8px}img{display:block;width:100%;aspect-ratio:16/9;object-fit:contain;background:white}figcaption{padding:8px 4px 3px;line-height:1.35}</style></head><body><h1>Module 10 · ${slides.length}-slide visual QA</h1><div class="grid">${tiles}</div></body></html>`;
  await writeFile(join(ARTIFACT_DIRECTORY, 'contact-sheet.html'), html, 'utf8');
}

export async function renderM10Deck() {
  ...
  try {
    for (let index = 0; index < slides.length; index += 1) {
      ...
      const url = `http://127.0.0.1:${address.port}/decks/m10-deliver-infrastructure-gitops-human-approval.html?transition=none#/${index}${finalFragment}`;
      ...
      for (let fragmentIndex = 0; fragmentIndex < fragmentCounts[index]; fragmentIndex += 1) {
        const fragmentOutput = join(ARTIFACT_DIRECTORY, `slide-${number}-fragment-${String(fragmentIndex + 1).padStart(2, '0')}.png`);
        const fragmentUrl = `http://127.0.0.1:${address.port}/decks/m10-deliver-infrastructure-gitops-human-approval.html?transition=none#/${index}/0/${fragmentIndex}`;
        await execute(chrome, ['--headless=new', '--disable-gpu', '--hide-scrollbars', '--window-size=1280,720', '--virtual-time-budget=3000', '--run-all-compositor-stages-before-draw', `--screenshot=${fragmentOutput}`, fragmentUrl], {maxBuffer: 1024 * 1024});
        fragmentStateCount += 1;
      }
  ...
  console.log(`wrote ${fragmentStateCount} fragment-state screenshots`);
  console.log(`wrote ${ARTIFACT_DIRECTORY}/contact-sheet.html`);
}

const isCli = process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url);
if (isCli) await renderM10Deck();
